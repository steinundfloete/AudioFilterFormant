...
AudioConnection pc2(formant, 0, i2s1, 0);
AudioConnection pc3(formant, 0, i2s1, 1);

elapsedMillis paramTimer, cpuTimer;

void noteOn(uint8_t note, uint8_t vel) {
  float f = 440.0f * powf(2.0f, (note - 69) / 12.0f);
  osc.frequency(f);
  osc.amplitude(0.4f * (vel / 127.0f));
}
...
void setup() {
  ...
  formant.setQ(8.0f);
  formant.setFormantMorph(0.0f);
  formant.setBrightness(0.0f);
  formant.setBreath(0.20f);
  formant.setConsonant(0.25f, 0.10f, 0.08f);
}
...
void loop() {
  ...
  if (paramTimer > 400) {
    paramTimer = 0;
    float v  = random(0,400)  / 100.0f;
    float br = random(-120,120)/10.0f;
    float mo = random(0,100)  / 100.0f;
    formant.setVowel(v);
    formant.setBrightness(br);
    formant.setFormantMorph(mo);
  }

  if (cpuTimer > 1000) {
    cpuTimer = 0;
    Serial.printf("Alien CPU: %.2f%% max %.2f%%, Mem: %u\n",
